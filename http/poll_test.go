...
func BenchmarkGetPollBulk(b *testing.B) {
	pollMsg, err := insertDummyPoll()
	if err != nil {
		log.Fatal(err)
	}

	api := fmt.Sprintf(cEndpointFormat+"?id=%d", cAPIVersion, "polls", pollMsg.MetaData.ID)
	request, err := createAuthenticatedRequest("GET", api, nil)
	if err != nil {
		log.Fatal(err)
	}
