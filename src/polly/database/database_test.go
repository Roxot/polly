...
func TestUsers(t *testing.T) {
	...
	// test whether the user was inserted using the UserByEmail function
	gotUsr2, err := db.UserByEmail(newUsr1.Email)
	if err != nil {
		t.Fatal("Failed to find inserted user by email.")
	}

	// test whether the inserted user was inserted correctly
	if (gotUsr2.Id != newUsr1.Id) ||
		(gotUsr2.Email != newUsr1.Email) ||
		...
		t.Fatal("Retrieved user using UserByEmail didn't match inserted user.")
	}

	// test adding duplicate user
	newUsr3 := polly.PrivateUser{}
	newUsr3.Email = "0600112233"
	...
	if err == nil {
		t.Fatal("Was able to add a user with an already existing email.")
	}
	...
	if (gotUsr3.Id != newUsr2.Id) ||
		(gotUsr3.Email != newUsr2.Email) ||
		(gotUsr3.DisplayName != newUsr2.DisplayName) {
		t.Fatal("User found with PublicUserById did not match.")
	}

	// test public user by email
	gotUsr4, err := db.PublicUserByEmail(newUsr2.Email)
	if err != nil {
		t.Fatalf("Unable to retrieve public user by email: %s.", err)
	}

	// test whether they matched
	if (gotUsr4.Id != newUsr2.Id) ||
		(gotUsr4.Email != newUsr2.Email) ||
